...
# https://github.com/lostjared/ldd-deploy/blob/main/ldd_deploy.sh
...
# Extract DLL dependencies using ldd and filter out Windows references and Qt libraries
extract_dll() {
    local input_file="$1"
    local output_dir="$2"

    ldd "$input_file" | grep -vi windows | grep -v Qt | while IFS= read -r line; do
        copy_dll "$line"
    done
}
